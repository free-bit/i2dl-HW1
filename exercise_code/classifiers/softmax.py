...
def cross_entropy_loss_vectorized(W, X, y, reg):
    ...
    eps = (-np.max(y_hats, axis=1)).reshape(-1, 1)                   # Take maximum elements of each row, put eps in a column vector form (1xN)
    ...
    denom = np.sum(nom, axis=1).reshape(-1, 1)                       # Calculate denominator for softmax (1xN)
    softmax_scores = nom / denom                                     # Calculate softmax scores for every score (NxC)
    actual_class_scores = np.choose(y, softmax_scores.T)             # Pick softmax score of actual class for every sample: y_hats.T -> (CxN): Chooses the entry from each column vector wrt corresponding y entry
    loss = -np.mean(np.log(actual_class_scores)) + reg               # Calculate cross entropy loss: -sum(log(softmax))/n_samples
    softmax_scores[range(n_samples), y] -= 1                         # Subtract 1 from softmax score of each correct class for gradient calculation
    dW = np.mean(np.einsum('nd,nc->ndc', X, softmax_scores), axis=0) # Calculate mean of N-many dWs (DxC): Outer product of every row vector from X and softmax_scores
